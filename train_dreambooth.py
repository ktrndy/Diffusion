...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_prompts = pd.read_csv(instance_prompts_path).set_index('file_name').to_dict()['text']
    ...
    def __getitem__(self, index):
        ...
        file_name = self.instance_images_path[index % self.num_instance_images].name
        if not instance_image.mode == "RGB":
            instance_image = instance_image.convert("RGB")
        example["instance_images"] = self.image_transforms(instance_image)
        example["instance_prompt_ids"] = self.tokenizer(
            self.instance_prompts[file_name],
            truncation=False,
            padding="max_length",
...
def main(args):
    ...
    train_dataset = DreamBoothDataset(
        ...
        instance_prompts_path=args.instance_prompts_path,
    ...
    assert len(train_dataset) >= 50, "ÐÑƒÐ¶Ð½Ð¾ Ð¼Ð¸Ð½Ð¸Ð¼ÑƒÐ¼ 50 Ð¸Ð·Ð¾Ð±Ñ€Ð°Ð¶ÐµÐ½Ð¸Ð¹ Ð´Ð»Ñ Ð¾Ð±ÑƒÑ‡ÐµÐ½Ð¸Ñ"
